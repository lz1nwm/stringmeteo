...
ggplot(data = dt.nimh.s[STATION_ID %in% stations]) +
    ...
    lemon::facet_rep_wrap(. ~ paste0(STATION_NAME,' (',STATION_ID,'), ', max(dt.nimh.s[ddate != Sys.Date(), ddate])), 
               scales = 'free_y',
...
ggplot(dt.nimh.w[STATION_ID %in% stations & ddate2 %between% week_dates]) +
    ...
    lemon::facet_rep_wrap(. ~ paste0(STATION_NAME,' (',STATION_ID,'), ', max(dt.nimh$ddate)), 
                          scales = 'free_y',
                          repeat.tick.labels = T,
                          ncol = 2)+
    theme(axis.text = element_text(colour = 'black'),
