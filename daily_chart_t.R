...
dt.nimh.s[, tavg_ano := sum(tavg, -tavg_hist, na.rm = T), by = .(STATION_ID,STATION_NAME,ddate)]
dt.nimh.s[, tavg_ano_7d := frollmean(tavg_ano, n = 28, na.rm = T), by = .(STATION_ID,STATION_NAME)]

dt.nimh.s[, tavg_ano_max := sum(tavg, -tmax_hist, na.rm = T), by = .(STATION_ID,STATION_NAME,ddate)]
dt.nimh.s[, tavg_ano_max_7d := frollmean(tavg_ano_max, n = 28, na.rm = T), by = .(STATION_ID,STATION_NAME)]

dt.nimh.s[, tavg_ano_min := sum(tavg, -tmin_hist, na.rm = T), by = .(STATION_ID,STATION_NAME,ddate)]
dt.nimh.s[, tavg_ano_min_7d := frollmean(tavg_ano_min, n = 28, na.rm = T ), by = .(STATION_ID,STATION_NAME)]


dt.nimh.s[STATION_ID == '15614' & ddate_last >= '2026-06-17']
